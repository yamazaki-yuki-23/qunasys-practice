...
export default function Home(props: {
  ...
  React.useEffect(() => {
    ...
    const cereals = props.cereals
      .filter((cereal: cerealType) => {
        return (
          (!searchMfrKey || searchMfrKey === cereal['mfr']) &&
  ...
  }, [props.cereals, xAxis, yAxis, searchMfrKey, searchTypeKey]);
  return (
    <>
      <Header />
      <main>
        ...
        {/* X軸 */}
        <Axis setAxisVal={setXAxis} axisType={'X'} notSelectedAxisVal={yAxis} />
        {/* Y軸 */}
        <Axis setAxisVal={setYAxis} axisType={'Y'} notSelectedAxisVal={xAxis} />
        <h4 className={styles.SearchContext}>
          グラフに表示するデータの絞り込み
        </h4>
        <Search
...
export async function getServerSideProps(ctx: {
  req: { headers: { referer: string; host: string } };
}) {
  // データ取得URLの生成
  ...
  const uniqMfrArr: string[] = cereals
    .filter((el: cerealType, ind: number, self: cerealType[]) => {
      return self.findIndex((e) => e.mfr === el.mfr) === ind;
    })
    .map((cereal) => cereal['mfr']);

  const uniqTypeArr: string[] = cereals
    .filter((el: cerealType, ind: number, self: cerealType[]) => {
